...
//     {/* <TouchableOpacity onPress={()=>navigation.navigate('UserProfile',{ item: item,group_name:group_name})}> */}
...
const GroupMembers = () => {
  ...
  const navigation = useNavigation(); 
  const route = useRoute();
  const {group_picture,group_name,participants } = route.params;

  const userProfilePic=group_picture
  const renderItem = ({ item }) => ( 
 
    <TouchableOpacity onPress={()=>navigation.navigate('UserProfile',{user_id:item.user_id,group_name:group_name})}>
    {/* <TouchableOpacity onPress={()=>navigation.navigate('UserProfile',{ item: item,group_name:group_name})}> */}
    <View style={styles.notificationItem}>
    <Image
        source={
